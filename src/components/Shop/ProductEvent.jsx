...
const ProductEvent = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const data = await productAPI.getProductsByCategory(2); // Assuming category ID 2 is for "Eventos"
        setProducts(data);
        setLoading(false);
      } catch (err) {
  ...
  const totalPages = Math.ceil((filteredProducts.length > 0 ? filteredProducts : products).length / ITEMS_PER_PAGE);
